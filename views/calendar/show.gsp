...
<body>
	<div class="row">
		<div class="col-xs-12">
			<h1 class="page-header">${calendarInstance.name }</h1>
		</div>
	</div>
	<div class="row">
		<div class="col-xs-8">
		...
		<div class="col-xs-4" id="sidebar"></div>
	</div>
      ...
      <div class="modal-body">
        <g:form class="form-horizontal" role="form" name="newEventForm" url="[action:'save', controller:'event']">
         <div class="form-group">
		  ...
		  <input type="hidden" name="calendar" value="${calendarInstance?.id}">
		</g:form>
      </div>
...
$("#fullCalendar").fullCalendar({
				...
				eventSources: [
						...
						{url: '${request.contextPath }/event/listEvents/${calendarInstance.id }'},
				...
				eventResize: function(event, delta, revertFunc) {
			      if (window.confirm("Are you sure you want to change this event?")) {
			       $.ajax ({
			        	url: '${request.contextPath }/event/resize/'+event.id,
			        	type: "POST",
			    ...
			    eventDrop: function(event, delta, revertFunc) {
					if (window.confirm("Are you sure you want to change this event?")) {
			        $.ajax ({
			        	url: "${request.contextPath }/event/drop/"+event.id,
			        	type: "POST",
			    ...
			    eventClick: function(calEvent, jsEvent, view) {
			    	$.ajax({
			    		url: '${request.contextPath }/event/show/'+calEvent.id,
			    		type: 'GET',
		...
		$("#addEvent").on('click',function(){$("#newEventForm").submit();});
	</g:javascript>
</body>
